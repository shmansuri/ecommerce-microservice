...
@router.post('/create', response_model=productDictResponse)
async def create_image_router(data:ProductImageCreate, db:Session = Depends(get_db)):
    return create_image_service(db, data)
...
@router.put('/update/{image_id}', response_model=ProductImageUpdate)
async def update_image_router(data, db:Session = Depends(get_db)):
    pass

@router.delete('/delete/{image_id}', response_model=productDictResponse)
